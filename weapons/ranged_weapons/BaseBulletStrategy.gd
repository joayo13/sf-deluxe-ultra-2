...
var projectile_damage_min: int
var projectile_damage_max: int
...
func _on_area_2d_area_entered(area) -> void:
	if area is Hurtbox:
		...
		attack.damage = randi_range(projectile_damage_min, projectile_damage_max)
		attack.position = global_position
		attack.knockback = 0.0
		if randi_range(1, 100)< self.projectile_crit_chance:
			attack.damage = attack.damage * self.projectile_crit_multiplier
